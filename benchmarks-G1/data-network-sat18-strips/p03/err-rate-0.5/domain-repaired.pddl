(define (domain data-network)
  ...
  (:action release
	...
	:effect (and
		...
		(not (usage ?s ?before))
		(usage ?s ?after)
		(sum ?before ?size ?after)(increase (total-cost ) 0)))
  ...
  (:action send
	...
	:effect (and
		...
		(not (usage ?to ?before))
		(usage ?to ?after)
		(not (less-equal ?size ?limit))
		(less-equal ?size ?limit)(increase (total-cost ) (send-cost ?from ?to ?size))))
  ...
  (:action process
	...
	:effect (and
		...
		(not (usage ?s ?before))
		(usage ?s ?after)
		(not (sum ?limit ?before ?after))(increase (total-cost ) (process-cost ?sc ?s)))))