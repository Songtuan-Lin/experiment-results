(define (domain data-network)
  ...
  (:action release
	...
	:effect (and
		...
		(not (usage ?s ?before))
		(usage ?s ?after)
		(sum ?size ?before ?size)(increase (total-cost ) 0)))
  ...
  (:action send
	...
	:effect (and
		...
		(not (usage ?to ?before))
		(usage ?to ?after)
		(sum ?after ?after ?limit)(increase (total-cost ) (send-cost ?from ?to ?size))))
  ...
  (:action process
	...
	:effect (and
		...
		(not (usage ?s ?before))
		(usage ?s ?after)
		(script-io ?sc ?in2 ?in2 ?in1)(increase (total-cost ) (process-cost ?sc ?s)))))