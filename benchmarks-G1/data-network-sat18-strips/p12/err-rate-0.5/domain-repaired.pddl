(define (domain data-network)
  ...
  (:functions (total-cost) - number
	(process-cost ?sc - script) - number
	(send-cost ?from - server) - number
	(io-cost ?s - server) - number)
  (:action release
	:parameters (?d - data
		?s - server
		?size - numbers
	...
	:effect (and
		(not (cached ?d ?s))
		(not (usage ?s ?before))
		(usage ?s ?after)(increase (total-cost ) 0)))
  ...
  (:action save
	...
	:effect (and
		(saved ?d ?s)
		(usage ?s ?size)(increase (total-cost ) (io-cost ?s ?size))))
  ...
  (:action load
	...
	:effect (and
		...
		(not (usage ?s ?before))
		(usage ?s ?after)
		(sum ?size ?after ?before)(increase (total-cost ) (io-cost ?s ?size))))
  ...
  (:action send
	...
	:effect (and
		...
		(not (usage ?to ?before))
		(usage ?to ?after)
		(sum ?after ?after ?after)(increase (total-cost ) (send-cost ?from ?to ?size))))
  ...
  (:action process
	...
	:effect (and
		(cached ?out ?s)
		(not (usage ?s ?before))
		(usage ?s ?after)(increase (total-cost ) (process-cost ?sc ?s)))))