(define (domain agricola)
  ...
  (:constants num0 - num
	...
	tnormal - roundclass
	tharvest - roundclass
	harvest_init - phaseclass
	harvest_feeding - phaseclass
	harvest_breeding - phaseclass
	harvest_end - phaseclass
	sheep - animal
	boar - animal
	cattle - animal
	grain - vegetable
	...
	act_sheep - animaltag
	act_boar - animaltag
	act_cattle - animaltag
	backhome - roundparts
	renew - roundparts
	roundend - roundparts)
  (:predicates (next_stage ?s1 - stage ?s2 - stage)
  ...
  (:action ag__harvest_collecting_veg
	...
	:effect (and
		...
		(not (sown_veg ?v))
		(not (can_harvest ?v))
		(next2_num ?i2 ?i3)(increase (total-cost ) 1)))
  ...
  (:action ag__harvest_collecting_fromoven
	...
	:effect (and
		...
		(not (sown_veg ?v))
		(not (can_harvest ?v))
		(plowed_fields )(increase (total-cost ) 1)))
  ...
  (:action ag__harvest_feed
	...
	:effect (and
		...
		(can_breed boar)
		(can_breed cattle)
		(not (next_num ?irest ?ifeed))(increase (total-cost ) 1)))
  ...
  (:action ag__finish_round_backhome
	...
	:effect (and
		...
		(current_round ?r)(increase (total-cost ) 1)))
  ...
  (:action ag__finish_round_backhome_withchild
	...
	:effect (and
		...
		(not (newborn ))
		(hold_round ?r backhome)
		(next_round ?r ?r)(increase (total-cost ) 1)))
  ...
  (:action ag__advance_round_normal
	...
	:effect (and
		...
		(open_action ?act)
		(current_round ?r2)(increase (total-cost ) 1)))
  ...
  (:action take_food
	...
	:effect (and
		...
		(not (num_food ?i1))
		(num_food ?i2)
		(plowed_fields )(increase (total-cost ) (group_worker_cost ?wmax))))
  ...
  (:action build_fences
	...
	:effect (and
		...
		(current_worker ?w2)
		(fences_for ?a)
		(current_worker ?w1)(increase (total-cost ) (group_worker_cost ?wmax))))
  ...
  (:action collect_animal
	...
	:effect (and
		...
		(not (current_worker ?w1))
		(current_worker ?w2)
		(owned_animals ?a)(increase (total-cost ) (group_worker_cost ?wmax))))
  ...
  (:action collect_resource
	...
	:effect (and
		...
		(current_worker ?w2)
		(stored_resource ?res)
		(max_worker ?w1)(increase (total-cost ) (group_worker_cost ?wmax))))
  ...
  (:action build_room
	...
	:effect (and
		...
		(not (next_worker ?wmax ?wmax))(increase (total-cost ) (group_worker_cost ?wmax))))
  ...
  (:action family_growth
	...
	:effect (and
		...
		(current_worker ?w2)
		(newborn )
		(built_rooms ?room ?wnewmax)(increase (total-cost ) (group_worker_cost ?wmax))))
