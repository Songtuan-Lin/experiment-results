(define (domain pipesworld_strips)
  ...
  (:types object
	...
	product - object
	batch-atom - object)
  (:constants lco - product
	gasoleo - product
	rat-a - product
	oca1 - product
	oc1b - product)
  (:predicates (connect ?from - area ?to - area ?pipe - pipe)
	(unitary ?pipe - pipe)
  ...
  (:action pop-start
	...
	:effect (and
		...
		(not (is-product ?last-batch-atom ?product-batch-atom-in))
		(is-product ?last-batch-atom oc1b)))
  ...
  (:action push-unitarypipe
	...
	:effect (and
		...
		(not (may-interface ?product-first-batch ?product-first-batch))))
