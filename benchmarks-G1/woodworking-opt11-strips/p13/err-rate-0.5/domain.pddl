...
(:action do-glaze
	...
	:effect (and
		...
		(not (colour ?x natural))
		(colour ?x ?newcolour)
		(not (colour ?x ?newcolour))(increase (total-cost ) (glaze-cost ?x))))
...
(:action do-plane
	...
	:effect (and
		...
		(not (colour ?x ?oldcolour))
		(colour ?x natural)
		(not (unused ?x))(increase (total-cost ) (plane-cost ?x))))
...
(:action do-saw-small
	...
	:effect (and
		...
		(treatment ?p untreated)
		(boardsize ?b ?size_after)
		(not (boardsize-successor ?size_before ?size_before))(increase (total-cost ) 30)))
