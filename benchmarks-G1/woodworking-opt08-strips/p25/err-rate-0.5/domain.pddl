...
(:action do-spray-varnish
	...
	:effect (and
		...
		(not (colour ?x natural))
		(colour ?x ?newcolour)
		(not (is-smooth ?surface))(increase (total-cost ) (spray-varnish-cost ?x))))
...
(:action do-saw-small
	...
	:effect (and
		...
		(treatment ?p untreated)
		(boardsize ?b ?size_after)
		(not (contains-part ?b ?p))(increase (total-cost ) 30)))
...
(:action do-saw-medium
	...
	:effect (and
		...
		(treatment ?p untreated)
		(boardsize ?b ?size_after)
		(not (boardsize-successor ?size_after ?s1))(increase (total-cost ) 30)))
