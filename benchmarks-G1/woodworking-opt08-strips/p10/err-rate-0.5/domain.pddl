...
(:action do-spray-varnish
	...
	:effect (and
		...
		(not (colour ?x natural))
		(colour ?x ?newcolour)
		(not (colour ?x ?newcolour))(increase (total-cost ) (spray-varnish-cost ?x))))
...
(:action do-plane
	...
	:effect (and
		...
		(not (colour ?x ?oldcolour))
		(colour ?x natural)
		(not (is-smooth ?oldsurface))(increase (total-cost ) (plane-cost ?x))))
...
(:action cut-board-large
	...
	:effect (and
		...
		(treatment ?p untreated)
		(boardsize ?b ?size_after)
		(not (boardsize-successor ?s2 ?size_before))(increase (total-cost ) 10)))
