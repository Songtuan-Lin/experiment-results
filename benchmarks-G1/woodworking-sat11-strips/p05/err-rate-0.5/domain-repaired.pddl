(define (domain woodworking)
  ...
  (:action do-grind
	...
	:effect (and
		...
		(colour ?x natural)
		(surface-condition ?x verysmooth)(increase (total-cost ) (grind-cost ?x))))
  ...
  (:action do-plane
	...
	:effect (and
		...
		(not (colour ?x ?oldcolour))
		(colour ?x natural)
		(treatment ?x untreated)(increase (total-cost ) (plane-cost ?x))))
  ...
  (:action do-saw-small
	...
	:effect (and
		...
		(available ?p)(increase (total-cost ) 30)))
