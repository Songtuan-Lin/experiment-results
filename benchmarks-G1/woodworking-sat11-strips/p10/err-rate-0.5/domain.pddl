...
(:action do-grind
	...
	:effect (and
		...
		(colour ?x natural)
		(not (grind-treatment-change ?newtreatment ?oldtreatment))(increase (total-cost ) (grind-cost ?x))))
...
(:action cut-board-large
	...
	:effect (and
		...
		(treatment ?p untreated)
		(boardsize ?b ?size_after)
		(not (in-highspeed-saw ?b ?m))(increase (total-cost ) 10)))
...
(:action do-saw-large
	...
	:effect (and
		...
		(treatment ?p untreated)
		(boardsize ?b ?size_after)
		(not (boardsize-successor ?s2 ?s2))(increase (total-cost ) 30))) )