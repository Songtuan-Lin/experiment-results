(define (domain woodworking)
  ...
  (:action do-plane
	...
	:effect (and
		...
		(not (colour ?x ?oldcolour))
		(colour ?x natural)
		(not (grind-treatment-change ?oldtreatment ?oldtreatment))
		(grind-treatment-change ?oldtreatment untreated)(increase (total-cost ) (plane-cost ?x))))
  ...
  (:action do-saw-small
	...
	:effect (and
		...
		(treatment ?p untreated)
		(boardsize ?b ?size_after)
		(not (boardsize-successor ?size_after ?size_before))
		(boardsize-successor ?size_after ?size_before)(increase (total-cost ) 30)))
