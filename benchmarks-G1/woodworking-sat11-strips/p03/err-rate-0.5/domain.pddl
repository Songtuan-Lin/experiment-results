(define (domain woodworking)
  ...
  (:action do-immersion-varnish
	...
	:effect (and
		...
		(colour ?x ?newcolour)
		(not (colour ?x ?newcolour))(increase (total-cost ) 10)))
...
(:action do-grind
	...
	:effect (and
		...
		(colour ?x natural)
		(not (grind-treatment-change ?oldtreatment ?newtreatment))(increase (total-cost ) (grind-cost ?x))))
...
(:action cut-board-large
	...
	:effect (and
		...
		(not (boardsize-successor ?s2 ?s1))(increase (total-cost ) 10)))
...
(:action do-saw-medium
	...
	:effect (and
		...
		(treatment ?p untreated)
		(boardsize ?b ?size_after)
		(not (boardsize ?b ?size_before))(increase (total-cost ) 30)))
