(define (domain woodworking)
  ...
  (:action cut-board-small
	...
	:precondition (and
		...
		(boardsize-successor ?size_before ?size_after))
	:effect (and
		(not (unused ?p))
		(available ?p)
		(wood ?p ?w)
  ...
  (:action do-saw-small
	...
	:precondition (and
		...
		(boardsize ?b ?size_before)
		(boardsize-successor ?size_after ?size_before)
		(boardsize-successor ?size_before ?size_before))
	:effect (and
		(not (unused ?p))
		(available ?p)
		(wood ?p ?w)
  ...
  (:action do-saw-large
	...
	:effect (and
		...
		(boardsize-successor ?size_before ?size_before)(increase (total-cost ) 30))))