...
(:action do-glaze
	...
	:effect (and
		...
		(not (colour ?x natural))
		(colour ?x ?newcolour)
		(not (unused ?x))(increase (total-cost ) (glaze-cost ?x))))
...
(:action do-plane
	...
	:effect (and
		...
		(not (colour ?x ?oldcolour))
		(colour ?x natural)
		(not (grind-treatment-change ?oldtreatment ?oldtreatment))(increase (total-cost ) (plane-cost ?x))))
...
(:action unload-highspeed-saw
	...
	:effect (and
		...
		(empty ?m)
		(not (empty ?m))(increase (total-cost ) 10)))
...
(:action cut-board-small
	...
	:effect (and
		...
		(not (boardsize-successor ?size_before ?size_after))(increase (total-cost ) 10)))
...
(:action cut-board-medium
	...
	:effect (and
		...
		(treatment ?p untreated)
		(boardsize ?b ?size_after)
		(not (boardsize-successor ?s1 ?s1))(increase (total-cost ) 10)))
...
(:action cut-board-large
	...
	:effect (and
		...
		(treatment ?p untreated)
		(boardsize ?b ?size_after)
		(not (boardsize-successor ?size_before ?s2))(increase (total-cost ) 10)))
...
(:action do-saw-small
	...
	:effect (and
		...
		(treatment ?p untreated)
		(boardsize ?b ?size_after)
		(not (boardsize-successor ?size_after ?size_before))(increase (total-cost ) 30)))
