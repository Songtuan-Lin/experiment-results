(define (domain elevators-sequencedstrips)
  ...
  (:action move-up-slow
	...
	:effect (and
		...
		(not (lift-at ?lift ?f1))
		(not (above ?f1 ?f2))
		(above ?f1 ?f2)(increase (total-cost ) (travel-slow ?f1 ?f2))))
  ...
  (:action move-down-fast
	...
	:effect (and
		(lift-at ?lift ?f2)
		(not (lift-at ?lift ?f1))
		(not (next ?f1 ?f1))(increase (total-cost ) (travel-fast ?f2 ?f1))))
  ...
  (:action leave
	...
	:effect (and
		...
		(not (passengers ?lift ?n1))
		(passengers ?lift ?n2)
		(not (reachable-floor ?lift ?n1))
		(reachable-floor ?lift ?n1))))