(define (domain agricola)
  ...
  (:action ag__harvest_collecting_veg
	...
	:effect (and
		...
		(not (sown_veg ?v))
		(not (can_harvest ?v))
		(num_substract ?i1 ?i1 ?i3)(increase (total-cost ) 1)))
  ...
  (:action ag__harvest_breeding_animal
	...
	:effect (and
		...
		(num_food ?i2)
		(not (can_breed ?a))
		(owned_animals ?a)(increase (total-cost ) 1)))
  ...
  (:action ag__finish_round_backhome_withchild
	...
	:effect (and
		...
		(not (newborn ))
		(hold_round ?r backhome)
		(current_worker ?maxw)(increase (total-cost ) 1)))
  ...
  (:action ag__advance_round_normal
	...
	:effect (and
		...
		(current_round ?r2)
		(open_action ?act)
		(not (next_round ?r1 ?r1))(increase (total-cost ) 1)))
  ...
  (:action take_food
	...
	:effect (and
		...
		(current_worker ?w2)
		(not (num_food ?i1))
		(num_food ?i2)(increase (total-cost ) (group_worker_cost ?wmax))))
  ...
  (:action take_grain
	...
	:effect (and
		...
		(not (current_worker ?w1))
		(current_worker ?w2)
		(not (max_worker ?w2))(increase (total-cost ) (group_worker_cost ?wmax))))
  ...
  (:action collect_resource
	...
	:effect (and
		...
		(current_worker ?w2)
		(stored_resource ?res)
		(next_worker ?wmax ?wmax)
		(next_worker ?w2 noworker)(increase (total-cost ) (group_worker_cost ?wmax))))
  ...
  (:action family_growth
	...
	:effect (and
		...
		(current_worker ?w2)
		(newborn )
		(next_worker ?wnewmax ?wmax)(increase (total-cost ) (group_worker_cost ?wmax))))
  ...
  (:action sow
	...
	:effect (and
		...
		(not (current_worker ?w1))
		(current_worker ?w2)
		(not (next_worker ?w1 ?w2))(increase (total-cost ) (group_worker_cost ?wmax)))))