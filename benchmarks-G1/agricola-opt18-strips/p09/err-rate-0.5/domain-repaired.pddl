(define (domain agricola)
  ...
  (:action ag__harvest_collecting_veg
	...
	:effect (and
		...
		(not (sown_veg ?v))
		(not (can_harvest ?v))
		(next_num ?i3 ?i1)(increase (total-cost ) 1)))
  ...
  (:action ag__harvest_feed
	...
	:effect (and
		...
		(can_breed boar)
		(can_breed cattle)
		(next2_num ?inow ?ifeed)(increase (total-cost ) 1)))
  ...
  (:action collect_animal
	...
	:effect (and
		...
		(current_worker ?w2)
		(owned_animals ?a)
		(next_worker ?wmax ?wmax)(increase (total-cost ) (group_worker_cost ?wmax))))
  ...
  (:action collect_cook_animal
	...
	:effect (and
		...
		(num_food ?i2)
		(ok )
		(food_required ?w2 ?i2)(increase (total-cost ) (group_worker_cost ?wmax))))
