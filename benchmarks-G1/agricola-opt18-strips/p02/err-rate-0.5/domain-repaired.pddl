(define (domain agricola)
  ...
  (:action ag__finish_stage
	...
	:effect (and
		...
		(open_action ?act)
		(drawcard_round ?act ?r1)(increase (total-cost ) 1)))
  ...
  (:action take_food
	...
	:effect (and
		...
		(not (num_food ?i1))
		(num_food ?i2)
		(next2_num ?i1 ?i2)(increase (total-cost ) (group_worker_cost ?wmax))))
  ...
  (:action collect_resource
	...
	:effect (and
		...
		(current_worker ?w2)
		(stored_resource ?res)
		(supply_resource ?act ?res)(increase (total-cost ) (group_worker_cost ?wmax))))
