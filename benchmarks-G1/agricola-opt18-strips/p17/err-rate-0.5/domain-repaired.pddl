(define (domain agricola)
  ...
  (:action ag__harvest_collecting_fromoven
	...
	:effect (and
		...
		(not (sown_veg ?v))
		(not (can_harvest ?v))
		(stored_veg ?v)(increase (total-cost ) 1)))
  ...
  (:action ag__harvest_feed
	...
	:effect (and
		...
		(can_breed boar)
		(can_breed cattle)
		(num_substract ?inow ?ifeed ?inow)(increase (total-cost ) 1)))
  ...
  (:action ag__advance_round_normal
	...
	:effect (and
		...
		(current_round ?r2)
		(open_action ?act)
		(available_action ?act)(increase (total-cost ) 1)))
  ...
  (:action collect_animal
	...
	:effect (and
		...
		(current_worker ?w2)
		(owned_animals ?a)
		(max_worker ?w2)(increase (total-cost ) (group_worker_cost ?wmax))))
  ...
  (:action build_room
	...
	:effect (and
		...
		(next_worker ?w1 ?wmax)(increase (total-cost ) (group_worker_cost ?wmax))))
  ...
  (:action family_growth
	...
	:effect (and
		...
		(current_worker ?w2)
		(newborn )
		(next_worker ?wnewmax ?w2)(increase (total-cost ) (group_worker_cost ?wmax))))
