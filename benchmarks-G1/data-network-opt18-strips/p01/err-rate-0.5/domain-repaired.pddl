(define (domain data-network)
  ...
  (:action release
	...
	:effect (and
		(not (cached ?d ?s))
		(not (usage ?s ?before))(increase (total-cost ) 0)))
  ...
  (:action load
	...
	:effect (and
		(not (usage ?s ?before))
		(usage ?s ?after)
		(cached ?d ?s)(increase (total-cost ) (io-cost ?s ?size))))
  ...
  (:action send
	...
	:effect (and
		...
		(not (usage ?to ?before))
		(usage ?to ?after)
		(usage ?to ?before)(increase (total-cost ) (send-cost ?from ?to ?size))))
  ...
  (:action process
	...
	:effect (and
		...
		(not (usage ?s ?before))
		(usage ?s ?after)
		(not (sum ?before ?size ?after))(increase (total-cost ) (process-cost ?sc ?s)))))